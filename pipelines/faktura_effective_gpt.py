...
def pipeline(df, df_fakt, row, idx, model, config):
    try:
        ...
        first_res, chat = add_message(chat, first_prompt, idx, model, {'max_tokens': 2500}, 1)
        first_res_dec = gpt_oss_message_decoder(first_res)
        output['label'] = classify(first_res_dec, ['Tjenesteydelse', 'Materialeindkøb'])
        ...
        second_res, chat, stats = add_message(chat, second_prompt, idx, model, {'max_tokens': 2500}, 1, stats=True)
        second_res_dec = gpt_oss_message_decoder(second_res)
        output['reason'] = second_res_dec


        # Query score
        third_prompt = format_prompt(prompts[3], input_dict)
        third_res, chat = add_message(chat, third_prompt, idx, model, {'max_tokens': 2500}, 1)
        third_res_dec = gpt_oss_message_decoder(third_res)
        output['secure'] = first_number(third_res_dec)
...
if __name__ == '__main__':
    ...
    config_path = '../config/faktura_effective_gpt.yaml'
