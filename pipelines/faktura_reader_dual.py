...
def pipeline(row, idx, model_vl, model, config, prompt, temp):
    try:
        pdf = pdf_path(row, config)
        images = pdf_to_images(pdf, temp)
        result_raw = response_image_input(prompt=prompt, idx=idx, model=model_vl, images=images, config=config, log_every=1)
...
if __name__ == '__main__':
    ...
    config_path = '../config/faktura_reader_dual.yaml'
    # Load config file
    ...
    model_vl = lms.llm(config['model_vl'])
    logging.info(f'Model_VL {config['model_vl']} succesfully loaded')
    ...
    for idx in range(idx, len(df)):
        result = pipeline(df.loc[idx], idx, model_vl, model, config, prompt, temp_path)
