...
def pipeline(df, row, idx, model, config):
    try:
        ...
        res1, chat = add_message(chat, prompt1, idx, model, {'max_tokens': 2000}, -1)
        res1_cleaned = gpt_oss_message_decoder(res1)
        output['label'] = classify(res1_cleaned, ['Tjenesteydelse', 'Materialeindkøb'])

        dict2 = create_dictionary(df, row, config, {'klassificering': output['label']})
        prompt2 = format_prompt(prompts[2], dict2)
        res2, chat, stats = add_message(chat, prompt2, idx, model, {'max_tokens': 2000}, 1, stats=True)
        res2_cleand = gpt_oss_message_decoder(res2)
        output['reason'] = res2_cleand

        prompt3 = format_prompt(prompts[3], dict2)
        res3, chat = add_message(chat, prompt3, idx, model, {'max_tokens': 2000}, -1)
        res3_cleaned = gpt_oss_message_decoder(res3)
        output['secure'] = first_number(res3_cleaned)


        prompt4 = format_prompt(prompts[4], dict2)
        res4, chat = add_message(chat, prompt4, idx, model, {'max_tokens': 2000}, -1)
        res4_cleaned = gpt_oss_message_decoder(res4)
        output['category'] = first_number(res4_cleaned)
...
if __name__ == '__main__':
    ...
    config_path = '../config/faktura_classifier_gpt.yaml'
